...
void fill_flat_bottom_triangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
    ...
    // slope is inverse, since I'm descending down the slope
...
void draw_filled_triangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
    ...
    // this if/elseif is to eliminate possibility of dividing by zero
    if(y1 == y2)
    {
        // only flat-bottom triangle needed
        fill_flat_bottom_triangle(x0, y0, x1, y1, x2, y2, color);
    }
    else if(y0 == y1)
    {
        // only flat-top triangle needed
...
void draw_texel(
    ...
    // variables to store the interpolated values of u, v, and 1/w for pixel
    float interpolated_u;
    float interpolated_v;
    float interpolated_reciprocal_w;

    // perform interpolation of all U/w, V/w values using barycentric weights
    ...
    )
{
    ...
    // flip the V component to account for inverted UV-coordinates (V grows down)
    v0 = 1.0 - v0;
    v1 = 1.0 - v1;
    v2 = 1.0 - v2;
